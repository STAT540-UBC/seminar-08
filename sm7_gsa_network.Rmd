...
1. Testing the over-representation of gene sets for your list of top significant genes

2. Score-based Gene Set Enrichment Analysis based on your list of gene-level scores
...
require(R.utils)           # need this to handle gzipped data
...
* [Part 2: When we want to use gene-level scores to rank the importance](#part-2-score-based-gene-set-enrichment-analysis)
...
Let's use the same data used in [`seminar-05`](https://github.com/STAT540-UBC/seminar-05/blob/main/sm5_differential_expression_analysis.md). You can repeat the same code of `seminar-05`, but we copied that for completeness (only in `Rmd`).
...
We can download public data mapping SNPs (genes) to diseases/phenotypes from [the NHGRI-EBI GWAS Catalog](https://www.ebi.ac.uk/gwas/). The GWAS catalogue file is already processed and stored in this repository for your convenience, but you're welcome to try out the code.
...
We may observe overlap $q^{\star}$ genes by random sampling of $k$ genes **without** replacement.
...
Remember the null data generating process of the hypergeometric distribution: a uniform sampling of balls in urns without replacement. Well, the "without replacement" part makes sense because we don't draw a DEG more than once in our analysis. However, the "uniform sampling" part may not hold in practice considering that some balls can be bigger than the other. Those bigger balls can be coloured differently.
...
[Gene Size Matters: An Analysis of Gene Length in the Human Genome](https://www.frontiersin.org/articles/10.3389/fgene.2021.559998/full)
...
> supportedGenomes or supportedGeneIDs, length data is available in
> the local database for our gene ID, “ensGene” and our genome,
> "hg19". We will let goseq automatically fetch this data from its
> databases."
...
The original Gene Set Enrichment Analysis method is not so scalable and gunky in permutation steps. To calibrate low P-values, e.g., 1e-6, you would literally need to sample a million permutations for all the gene sets (with some exaggeration). The authors of `fgsea` come up with smart computation tricks and approximate p-value calculation quite accurately. You can check out this preprint: [https://www.biorxiv.org/content/10.1101/060012v3](https://www.biorxiv.org/content/10.1101/060012v3)
...
[https://github.com/ctlab/fgsea](https://github.com/ctlab/fgsea)

```{r eval=FALSE}
remotes::install_github("ctlab/fgsea")
```

*Remark on a technical issue*: You need `C++` compiler installed in
your machine if you want to use this package with best performance.
...
* Run `fgsea` or  [`GSEABase`](https://bioconductor.org/packages/release/bioc/html/GSEABase.html) for the same DEGs testing the interaction effect (0.5 pt).
